...
public class TestAstar {
	...
	public TestAstar(List<Point2D> openList,Point robotLocation, Point finishPoint, List<Point2D> wallOfset){
		...
		while(!openList.isEmpty()){
				...
				neigh1 = new Point(lastPoint.x - 1, lastPoint.y - 1); 				neigh2 = new Point(lastPoint.x, lastPoint.y - 1);
				neigh3 = new Point(lastPoint.x + 1, lastPoint.y - 1); 				neigh4 = new Point(lastPoint.x + 1, lastPoint.y);
				neigh5 = new Point(lastPoint.x + 1, lastPoint.y + 1); 				neigh6 = new Point(lastPoint.x, lastPoint.y + 1);
				neigh7 = new Point(lastPoint.x - 1, lastPoint.y + 1); 				neigh8 = new Point(lastPoint.x - 1, lastPoint.y);
				
								
				double hval1 = countHvalue(neigh1.x, neigh1.y); 				double hval2 = countHvalue(neigh2.x, neigh2.y);
				double hval3 = countHvalue(neigh3.x, neigh3.y); 				double hval4 = countHvalue(neigh4.x, neigh4.y);
				double hval5 = countHvalue(neigh5.x, neigh5.y); 				double hval6 = countHvalue(neigh6.x, neigh6.y);
				double hval7 = countHvalue(neigh7.x, neigh7.y); 				double hval8 = countHvalue(neigh8.x, neigh8.y);
				
				double gval1 = countGvalue(neigh1.x, neigh1.y);				double gval2 = countGvalue(neigh2.x, neigh2.y);
				double gval3 = countGvalue(neigh3.x, neigh3.y);				double gval4 = countGvalue(neigh4.x, neigh4.y);
				double gval5 = countGvalue(neigh5.x, neigh5.y);				double gval6 = countGvalue(neigh6.x, neigh6.y);
				double gval7 = countGvalue(neigh7.x, neigh7.y);				double gval8 = countGvalue(neigh8.x, neigh8.y);
				
				double fval1 = hval1 + gval1;				double fval2 = hval2 + gval2;
				double fval3 = hval3 + gval3;				double fval4 = hval4 + gval4;
				double fval5 = hval5 + gval5;				double fval6 = hval6 + gval6;
				double fval7 = hval7 + gval7;				double fval8 = hval8 + gval8;
				
				temporaryFValsOfNeighbors.put(fval1, neigh1); temporaryFValsOfNeighbors.put(fval2, neigh2);				temporaryFValsOfNeighbors.put(fval3, neigh3); temporaryFValsOfNeighbors.put(fval4, neigh4); 
				temporaryFValsOfNeighbors.put(fval5, neigh5); temporaryFValsOfNeighbors.put(fval6, neigh6);				temporaryFValsOfNeighbors.put(fval7, neigh7); temporaryFValsOfNeighbors.put(fval8, neigh8);
				
				temporaryGvalsOfNeighbors.put(neigh1, gval1); temporaryGvalsOfNeighbors.put(neigh2, gval2);				temporaryGvalsOfNeighbors.put(neigh3, gval3); temporaryGvalsOfNeighbors.put(neigh4, gval4); 
				temporaryGvalsOfNeighbors.put(neigh5, gval5); temporaryGvalsOfNeighbors.put(neigh6, gval6);				temporaryGvalsOfNeighbors.put(neigh7, gval7); temporaryGvalsOfNeighbors.put(neigh8, gval8); 
